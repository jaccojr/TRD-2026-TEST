...
def shift():
    ...
    #     Also include each stage's alt.wxPoints halfway point (key suffixed "b"),
    #     since a stage with an A/B split has a genuinely different midpoint for B —
    #     dropping this here would silently regress the A/B weather fix. ---
    points = []
    hours = [8, 12, 16]
    for s in d["stages"]:
        ...
        if s.get("alt") and s["alt"].get("wxPoints"):
            alt_mid = s["alt"]["wxPoints"][1]  # only the halfway point differs from A
            points.append({
                "key": f"{s['num']}-1b",
                "lat": round(alt_mid["lat"], 4),
                "lon": round(alt_mid["lon"], 4),
                "ele": alt_mid["ele"],
                "date": s["iso"],
                "hour": hours[1],
            })
