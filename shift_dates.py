...
def shift():
    ...
    #     Keyed by each point's semantic label (Start/VP1/VP2/Finish), not its array
    #     position — a stage's wxPoints array can be length 3 or 4 depending on whether
    #     it has one or two intermediate points (Proloog and Etappe 5B have only one),
    #     so positional indexing breaks the moment stages don't all have the same count.
    ...
    points = []
    label_index = {"Start": 0, "VP1": 1, "VP2": 2, "Finish": 3}
    anchor_hour = {"Start": 8, "VP1": 11, "VP2": 14, "Finish": 16}
    for s in d["stages"]:
        for wx in s["wxPoints"]:
            idx = label_index[wx["label"]]
            points.append({
                "key": f"{s['num']}-{idx}",
                "lat": round(wx["lat"], 4),
                "lon": round(wx["lon"], 4),
