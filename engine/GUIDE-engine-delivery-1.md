# Engine delivery 1 (v0.1-PRELIMINARY): what goes where
...
| `engine/lang/*.json` | 6 language files: ui, sport-cycling and weather, each in NL and EN |
| `engine/weather-keymap.json` | Translates the weather keys (`1-2` → `s1-fs2`), so the engine reads the existing `weather.json` |
| `engine/sw.js` | The engine's own service worker (own cache `engine-trd2026-v0.1`, scope `/engine/` only) |
...
1. Unzip `engine-delivery-1.zip`. You'll get a folder `engine`.
2. Open **github.com/jaccojr/TRD-2026-TEST**.
3. Click **Add file → Upload files**.
4. Drag the **whole `engine` folder** onto the page (Chrome/Edge/Safari keep the subfolder `lang/`).
5. Check the list: it should show `engine/index.html`, `engine/lang/ui.nl.json` etc., with `engine/` in front.
6. Commit message, e.g. `engine delivery 1 (v0.1-PRELIMINARY)`, then **Commit changes**.
7. Wait 1–2 minutes for GitHub Pages.
...
Every later delivery bumps the cache name (`engine-trd2026-v0.2` …), the TRD lesson about stale files.
